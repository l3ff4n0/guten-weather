...
function guten_weather_guten_weather_block_init() {
    ...
    if(!empty($api_key)){
        register_block_type( __DIR__, array(
            ...
            'render_callback' => 'guten_weather_render_callback'
        ));
    }
}
add_action( 'init', 'guten_weather_guten_weather_block_init' );

function guten_weather_render_callback($attributes, $content) {
    ...
    if(empty($weather_api_key)){
        ...
        if($attributes['city'] != 'select your city' || !empty($attributes['city'])){
            ...
            $language = ($attributes['languageData'] != 'select your language') ? '&lang=' . $attributes['languageData'] : ''; 
            $weather_endpoint = ($attributes['weatherType'] == 'forecast') ? $weather_api . 'key='. $weather_api_key . '&q=' . $attributes['city'] .'&days='. $attributes['numberDays'] . $language : $weather_api . 'key='. $weather_api_key . '&q=' . $attributes['city'] . $language;
            $response = file_get_contents($weather_endpoint);
            ...
            if(is_array($weather_data)){
                ...
                $content = '<div class"widget-weather-container"  style="background:' . $attributes['WidgetBgColor'] . '; color: ' . $attributes['WidgetColor'] .'; border-radius:' . $attributes['borderRadius'].'px">';
                ...
                if($attributes['weatherType'] == 'forecast'){
                    ...
                    if(is_array($weather_forecast)){
                        $weatherWidget .= '<div class="weather-forecast-container">';
